...
**Shipped:** four self-contained, runnable crawlers under `samples/`, each against
...
JSONL, via `Spider.crawl`), `QuotesJs` (dynamic data, no browser — lifts an embedded
`var data = […]` array via `Extract.assignedJson`), and `PoliteRotation` (UA rotation →
`Spider.crawlPolitely`). Indexed in `docs/cookbook.md` and `samples/README.md`.
**Remaining recipes** (future additions, same pattern): login+session (`session-auth`) and
infinite-scroll (`dynamic-page`). The original build prompt is kept below for reference.
